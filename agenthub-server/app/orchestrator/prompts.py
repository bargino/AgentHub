...
8. pipeline 每个 task 尽量给 acceptance（EARS 句式"当<条件>，系统应<可观测结果>"，一句话、可验证）；
   无法明确时留空字符串
...
# item 4：规划→批判→修订。对高复杂度 pipeline 计划做一次自我评审 + 改进（1 次 LLM 调用）。
PLAN_REVISE_SYSTEM = """你是资深技术规划评审。给你一份针对用户需求生成的多步任务计划，请先批判、再改进。
批判维度：是否缺关键步骤、有无冗余步骤、顺序与依赖（dependsOn）是否合理、每步 acceptance 是否具体可验证、
agent 选型是否恰当、是否违背最小改动原则。然后输出改进后的计划（若原计划已很好，可与原计划一致）。
